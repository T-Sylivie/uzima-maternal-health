...
DB_NAME=uzima
DB_USER=uzima_user
DB_PASSWORD=<see credentials below>
DB_HOST=localhost
DB_PORT=5432
...
| Database password | *(fill in — not stored here, see personal notes)* |
| Host | `localhost` |
| Port | `5432` |
...
|---|---|---|---|
| `admin` | *(superuser — set via createsuperuser)* | Django superuser | N/A |
...
| `chw_new` | `newpass123` | CHW | village_cell=`Nyamirambo`, health_centre_id=`HC-003` (created via admin UI test) |
...
| 7 — District Reporting | ✅ Complete | Aggregate village report |
| Bonus — Admin account creation | ✅ Complete | System Admin can create CHW/Nurse/District Officer accounts via UI |

## Known Issues

- Android emulator (Pixel 3a, API 28) crashes silently on startup — root cause not identified. Real-device testing via USB is the workaround.
...
- District report is not scoped by actual district (no village-to-district mapping exists yet); currently returns all villages system-wide.
